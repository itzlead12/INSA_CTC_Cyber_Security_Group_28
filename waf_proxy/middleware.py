...
class PureWebSocketManager:
    ...
    def process_admin_api_data(self, api_data: Dict) -> Dict:
        ...
        traffic_chart_data = self.prepare_traffic_chart_data(requests_by_hour)  # ADD THIS LINE
        ...
        return {
            ...
            "charts_data": {
                "traffic_data": traffic_chart_data,  # Use the prepared data
    ...
    # FIX: Use prepare_traffic_chart_data for client data too
        ...
        traffic_chart_data = self.prepare_traffic_chart_data(traffic_data)  # ADD THIS LINE
...
class WAFMiddleware:
    ...
    def __init__(self, websocket_manager: PureWebSocketManager):
    ...
    async def process_request(self, request: Request, call_next):
        ...
        client_ip = self._get_client_ip(request)  # FIXED: Use proper IP detection
        ...
        # Perform WAF analysis (UPDATED: includes country and IP blocking)
...
"""
    ...
    async def _handle_blocked_request(self, request: Request, waf_result: WAFResult, 
        ...
        if waf_result.reason == "reCAPTCHA required":
            recaptcha_page = self._generate_recaptcha_page(client_config.get('client_name', 'unknown'), client_ip)
            return Response(content=recaptcha_page, status_code=403, media_type="text/html")
    
        # Return standard blocked page
        html_content = BLOCKED_PAGE_HTML.replace("{{REASON}}", waf_result.reason).encode('utf-8')
